      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
        ...
        <div className="space-y-6">
          <div className={`rounded-xl border overflow-hidden flex flex-col h-full min-h-[500px] ${isDarkMode ? 'bg-[#1e2330] border-green-500/30' : 'bg-white border-green-200 shadow-sm'}`}>
            ...
            {/* SỬA bg-white THÀNH bg-transparent VÀ CHUYỂN CHỮ THÀNH MÀU SÁNG */}
            <div className={`p-6 overflow-y-auto max-h-[800px] flex-1 custom-scrollbar ${isDarkMode ? 'bg-transparent text-gray-200' : 'bg-[#fcfcfc] text-black'}`}>
              {result ? (
                <div id="ai-result-content" dangerouslySetInnerHTML={{ __html: result }} className="prose max-w-none" />
              ) : (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
                  {/* SỬA bg-gray-100 THÀNH bg-gray-800 ĐỂ KHÔNG BỊ CỤC TRẮNG QUANH ICON */}
                  <div className={`p-6 rounded-full mb-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
                    <FileText size={48} className={isDarkMode ? 'text-gray-400' : 'text-gray-300'} />
                  </div>
                  <h4 className={`text-lg font-bold mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-400'}`}>Chưa có dữ liệu hiển thị</h4>
                  <p className={`text-center text-sm max-w-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
